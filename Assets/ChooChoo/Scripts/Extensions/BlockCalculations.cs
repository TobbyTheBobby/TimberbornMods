...
namespace ChooChoo
{
  public static class BlockCalculations
  {
      ...
      Orientation orientation)
    {
      ...
      {
        Block positionedBlock = BlockCalculations.PositionBlock(allBlocks[index], coordinates, orientation);
        positionedBlocks.Add(positionedBlock);
        BlockCalculations.GetBottomBlocks(positionedBlock, (ICollection<Block>) positionedBlocks);
      }
    }

    public static Vector3 Pivot(Vector3Int coordinates, Orientation orientation) => (Vector3) coordinates + orientation.ToPivotOffset();
