...
namespace ChooChoo.Wagons
{
    public class ObjectFollower : ITrackFollower
    {
        ...
        private bool ReachedLastPathCorner() => _pathCorners.Count > 0 &&
                                                _navigationService.InStoppingProximity(_pathCorners.Last().RouteCorners.Last(), _transform.position);
        ...
        private int PeekNextSubCornerIndex()
        {
            ...
            var flag3 = !_navigationService.InStoppingProximity(_transform.position,
                _pathCorners[_currentCornerIndex].RouteCorners[_nextSubCornerIndex]);
        ...
        {
            return movementMagnitude <= 0.05 ? movement : movement.normalized * 0.05f;
        }
