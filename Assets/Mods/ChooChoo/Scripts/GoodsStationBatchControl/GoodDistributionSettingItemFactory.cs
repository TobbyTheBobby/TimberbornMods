...
namespace ChooChoo.GoodsStationBatchControl
{
    public class GoodDistributionSettingItemFactory
    {
        ...
        public GoodDistributionSettingItem Create(GoodsStationDistributionSettings goodsStationDistributionSettings,
            GoodsStationGoodDistributionSetting goodsStationGoodDistributionSetting)
        {
            var visualElement = _visualElementLoader.LoadVisualElement("Game/BatchControl/GoodDistributionSettingItem");
            var importGoodIcon = _importGoodIconFactory.CreateImportGoodIcon(visualElement.Q<VisualElement>("ImportGoodIconWrapper"),
                goodsStationGoodDistributionSetting.GoodId);
            ...
            var visualElement1 = _visualElementLoader.LoadVisualElement("Game/BatchControl/PopulationDistributorBatchControlRowItem");
            var textField = visualElement1.Q<IntegerField>("MinimumValue");
            ...
            var importToggle =
                _importToggleFactory.Create(visualElement.Q<VisualElement>("ImportToggleWrapper"), goodsStationGoodDistributionSetting);
            visualElement.Q<ProgressBar>("FillRateProgressBar").ToggleDisplayStyle(false);
            var goodDistributionSettingItem = new GoodDistributionSettingItem(_inputService, visualElement, goodsStationGoodDistributionSetting,
                importGoodIcon, importToggle, textField, plusButton, minusButton);
        ...
        private void InitializeMinimumControls(
            ...
            GoodsStationGoodDistributionSetting goodsStationGoodDistributionSetting)
        {
            TextFields.InitializeIntegerField(minimumValue, goodsStationGoodDistributionSetting.MaxCapacity,
                afterEditingCallback: newValue => OnIntFieldChange(newValue, goodsStationGoodDistributionSetting));
            ...
            var immigrationToggle = root.Q<Toggle>("ImmigrationToggle");
            immigrationToggle.ToggleDisplayStyle(false);
            var emigrationToggle = root.Q<Toggle>("EmigrationToggle");
            emigrationToggle.ToggleDisplayStyle(false);
        }

        private static void OnIntFieldChange(int newValue, GoodsStationGoodDistributionSetting goodsStationGoodDistributionSetting)
        {
        ...
        private static void OnButtonClicked(int change, GoodsStationGoodDistributionSetting goodsStationGoodDistributionSetting)
        {
            var minimum = goodsStationGoodDistributionSetting.MaxCapacity + change;
